...
void setup()
{
  for (ledPin = 2; ledPin < 6; ledPin++) //依次设置1~6个数字引脚为输出模式
  { 
...
void loop()
{
  for (ledPin = 2; ledPin < 6; ledPin++) //每隔delayTime依次点亮1~6引脚相连的led灯
  {
  digitalWrite(ledPin, HIGH); //点亮ledPin引脚相连的led灯
  delay(delayTime);//延时delayTime
  }

  for (ledPin = 5; ledPin > 2; ledPin--) //每隔delayTime依次熄灭6~1引脚相连的led灯
  {
